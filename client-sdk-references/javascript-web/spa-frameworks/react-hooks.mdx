...
The `@powersync/react` package provides React hooks for use with the [JavaScript Web SDK](/broken) or [React Native SDK](/broken). These hooks are designed to support reactivity, and can be used to automatically re-render React components when query results update or to access PowerSync connectivity status changes.

<Card
    title='React hooks for PowerSync | PowerSync SDK API Reference'
    icon='book'
